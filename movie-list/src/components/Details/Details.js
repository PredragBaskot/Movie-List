...
const Details = (props) => {
  ...
  const Details = () => {
    ...
    console.log(entries);
    const i = 13;
     const array = entries.slice(0,i).concat(entries.slice(i+2, entries.length));
     const array2 = array.splice(9,1)
    return (
      array.map(([key, value]) => { 
        return (<li><h3>{`${key} : ${value}`}</h3></li>)
      }))
  }
  ...
  return (
    <div>
      ...
      <img className="slika" src={movieDetails.Poster === 'N/A' ? "https://i.redd.it/ds1luav7dl851.jpg" : movieDetails.Poster} alt="movie" />
